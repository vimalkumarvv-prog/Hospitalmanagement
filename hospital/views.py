...
def doctor_signup_view(request):
    ...
    if request.method=='POST':
        ...
        if userForm.is_valid() and doctorForm.is_valid():
            ...
            user.set_password(user.password)
            user.save()
            doctor=doctorForm.save(commit=False) #prevents from saving immediately in to the database. if false doesn't pass it prevent linking  the user and doctor
            doctor.user=user
            doctor=doctor.save()
            my_doctor_group = Group.objects.get_or_create(name='DOCTOR') #This line retrieves the group named 'DOCTOR' from the database if it exists. If the group doesn't exist, it creates the group.
            my_doctor_group[0].user_set.add(user) #my_admin_group[0] is the group instance. It contains the data about the 'ADMIN' group, such as its id, name, and related users.my_admin_group[1] is a boolean that indicates whether the group was created (True) or fetched from the database (False).
...
def get_unavailable_slots(request, doctor_name, appointment_date):
    appointments = Appointment.objects.filter(
        doctorName=doctor_name,
        appointmentDate=appointment_date
    )
    unavailable_slots = [appointment.appointmentTime for appointment in appointments]
